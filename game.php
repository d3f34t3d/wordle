...
function saveLeaderboard($leaderboard) {
    ...
    $data = json_encode(['leaderboard' => $leaderboard], JSON_PRETTY_PRINT);
    file_put_contents($file, $data);
}


function addScoreToLeaderboard($playerName, $winStreak) {
    $leaderboard = getLeaderboard();
    $playerExists = false;

    foreach ($leaderboard as &$entry) {
        if ($entry['name'] === $playerName) {
            if ($winStreak > $entry['streak']) {
                $entry['streak'] = $winStreak;
            }
            $playerExists = true;
            break;
        }
    ...
    if (!$playerExists) {
        $leaderboard[] = ['name' => $playerName, 'streak' => $winStreak];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($input === $word) {
        ...
        addScoreToLeaderboard($_SESSION['game_state']['playerName'], $_SESSION['game_state']['currentWinStreak']);
